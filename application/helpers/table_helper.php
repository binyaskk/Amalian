...
Gets the html table to manage people.
*/
function get_role_manage_table($role,$controller)
{
	$CI =& get_instance();
	...
	$headers = array('<input type="checkbox" id="select_all" />', 
	$CI->lang->line('roles_id'),
	$CI->lang->line('roles_name'),
    $CI->lang->line('common_edit'),
	...
	$table.='</tr></thead><tbody>';
	$table.=get_role_manage_table_data_rows($role,$controller);
	$table.='</tbody></table>';
	return $table;
}
...
Gets the html data rows for the people.
*/
function get_role_manage_table_data_rows($role,$controller)
{
	$CI =& get_instance();
	$table_data_rows='';
	
	foreach ($role->result() as $data_row) {
		$table_data_rows.=get_role_data_row($data_row,$controller);
	}
	
	if ($role->num_rows()==0) {
		$table_data_rows.="<tr><td colspan='10'><div class='warning_message' style='padding:7px;'>".$CI->lang->line('roles_nothing_to_display')."</div></tr></tr>";
	}
	
	return $table_data_rows;
}

function get_role_data_row($data_row,$controller)
{
	$CI =& get_instance();
	$controller_path=$CI->router->fetch_module()."/".$CI->router->fetch_class();
	$width = $controller->get_form_width();
    if ($data_row->role_id==1) {  //disable checkbox for admin
	    $table_data_row='<tr class="disabled_row">';
	    $table_data_row.="<td width='5%'><input type='checkbox'  disabled='disabled' id='user_$data_row->role_id' value='".$data_row->role_id."'/></td>";
	}
    else {
	    $table_data_row='<tr>';	
	    $table_data_row.="<td width='5%'><input type='checkbox' id='role_$data_row->role_id' value='".$data_row->role_id."'/></td>";
	}
	// Apply the xss_clean() of "security" library, which filtered data from passing through <script> tag.
	$table_data_row.='<td width="10%">'.character_limiter(html_escape($CI->security->xss_clean($data_row->role_id)),10).'</td>';
	$table_data_row.='<td width="15%">'.character_limiter(html_escape($CI->security->xss_clean($data_row->role_name)),10).'</td>';
...
Gets the html table to manage people.
*/
function get_role_manage_report_table($people,$controller)
{
	/*$CI =& get_instance();
	$table='<table class="tablesorter" id="sortable_table">';
...
Gets the html data rows for the people report.
*/
function get_role_manage_report_table_data_rows($people,$controller)
{
	/*$CI =& get_instance();
	$table_data_rows='';
...
function get_role_report_data_row($data_row,$controller)
{
	/*$CI =& get_instance();
	$controller_path=$CI->router->fetch_module()."/".$CI->router->fetch_class();
	$width = $controller->get_form_width();
